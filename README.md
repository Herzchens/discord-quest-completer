...
- **The rest of the installer hardening, same PR.** A `dist` directory is no longer enough to be treated as a usable Vencord build; all four desktop runtime files must be present and non-empty. The replacement is staged and rolled back if it cannot complete, and the recovery backup is refreshed from the current usable state rather than preserving a copy from the first install forever. Only updater processes living under a Discord install root are stopped, instead of everything named `Update.exe`. A client that was closed and cannot be reopened no longer causes a different Discord flavour to be launched at you. On the devbuild side: the target flavour now follows the client you are actually running, `-DiscordBranch` can name it explicitly, Node 25 and newer work without Corepack by reading the pnpm version Vencord asks for, builds are transactional with a hash stamp so a half-written `dist` is never promoted to known-good, and `app.asar` recovery has to prove it worked rather than assuming a rename succeeded.
- **The release packager now proves the bundle came from a clean source tree.** It used to scan the built renderer for markers of specific third-party plugins, which only ever caught the ones somebody had thought to list, and that is how a foreign plugin shipped inside two v4.9.x/v4.10.x bundles. It now requires the source tree behind the dist to contain no userplugin except `orionQuests` and refuses to package otherwise. Also from #71: Windows installer regression coverage, run in CI against Windows PowerShell 5.1 and PowerShell 7 across Node 22, 24 and 25.

### v4.10.8
- **The bundle installer has been dying halfway through on every run since June.** Found by [@mods-hd](https://github.com/mods-hd) while fixing something else, in [#70](https://github.com/nyxxbit/discord-quest-completer/pull/70). `INSTALL.cmd` had an unescaped `)` inside the error message under the copy step, and `cmd` parses a parenthesised block before deciding whether to run it, so the block ended early and the leftover `.` aborted the script whether or not the copy had actually failed. The files were already copied by then, so installs worked, but Discord was force-closed at step 1 and never reopened and none of the "now enable the plugin" instructions ever printed. From the user's side the tool killed Discord and did nothing. It shipped in every release from v4.9.3 to v4.10.7. Reproduced here against a pristine `main` with the branch not even taken, and again end to end after the fix.
- **The bundle installer only ever handled Discord Stable** ([#69](https://github.com/nyxxbit/discord-quest-completer/issues/69), reported by [@kxlt6969](https://github.com/kxlt6969), fixed by [@mods-hd](https://github.com/mods-hd)). It closed `Discord.exe` and reopened Stable's `Update.exe`, both hardcoded, so on a Canary or PTB machine it closed nothing and reopened nothing. The copy itself was always fine, because Vencord keeps one `dist` folder for every branch, which is also why a running Canary locks exactly the files a running Stable would. All three builds are closed now, the ones that were actually open are reopened, and when Discord was already shut it starts the only installed build rather than guessing between several. Verified over seven layouts with a stubbed process table, and the detection line checked against a live machine running Stable with Canary installed but closed.
- **Five things Orion was inferring that Discord answers directly.** All five came out of reading Discord's own quest code on canary 1.0.1133 rather than from a bug report. `detectType` picked a quest's task by whichever key the server happened to list first, so `startsWith("PLAY")` matched `PLAY_ON_XBOX` as readily as `PLAY_ON_DESKTOP`; 38 of 66 quests on a live account carry two or three keys, and a reordering would have meant injecting a desktop process for a console task and reading progress under a key Discord never credits, silently, for 25 minutes. Discord dispatches `QUESTS_SEND_HEARTBEAT_FAILURE` with the real error on every failed beat and Orion was not listening, so a failing game quest arrived as 90 seconds of silence followed by a guess; five consecutive failures now abort in about three seconds naming Discord's own status and error code. `questAccessSuspendedUntil` arrives beside the enrollment block in the same fetch and Discord's own client refuses to start a quest while it is set, so a suspended account was being hammered instead of told. Quest expiry was computed twice and differently by the two engines, and neither matched Discord's own answer, which both now ask for. And two account predicates were left on strict equality when the rest became tolerant of a momentary blank identity, which could strand a quest as `RUNNING` for the rest of a run.

### v4.10.7
- **`/orion start` told you it had started something it had already given up on.** Reported by [@4kema](https://github.com/4kema) in [#66](https://github.com/nyxxbit/discord-quest-completer/issues/66). The command answered `Started.` and returned, but a run that finds nothing left to farm is over a second later, so the next `/orion status` read `Idle` and the only explanation was a line in a console the plugin gives nobody a reason to open. It looked like the command did nothing, and [@Herzchens](https://github.com/Herzchens) hit the same thing from QuestUI's side, where the button appeared to bounce straight back. The engine now records why a run ended on its own, `start` reports it when the run ends before the reply does, and `status` says it instead of a flat `Idle`. Checking once after starting was tried and gets it right only about half the time, because the first scan is not reliably finished by then; polling until the engine stops was tried and cost 2.2 seconds on every healthy start, since a healthy start is the one case that never exits the poll early. It races the run against a 400ms deadline instead, so an empty run answers in about 13ms and a real one pays the deadline once.
- **The plugin now carries a version.** It never had one, so someone reporting a plugin bug could not say which build it came from, and neither could anyone reading the report. That is what stalled #66 for a day. It sits on the plugin object and at the front of every `/orion status` line, and the packaging script refuses to build if it drifts from the userscript's.
