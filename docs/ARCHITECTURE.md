...
This document describes how Orion is structured internally. It is intended for contributors and the curious, not as a user guide. Last reviewed against `index.js` **v4.11.2**.
...
The log line carries the task keys the quest offered, because "Unknown task type" told nobody anything, and a console-only quest reads differently from one whose task type has no handler here. Keys we know are validated outside the client sit in `UNAUTOMATABLE_KEYS` with the reason attached. `ACHIEVEMENT_IN_GAME` is the first of them, read off the Battlefield 6 Multiplayer quest in #78: it is not `ACHIEVEMENT_IN_ACTIVITY`, there is no embedded activity and no `discordsays` backend behind it, so the OAuth bypass has nothing to authorize against and the quest wants an achievement earned in the retail game. A run that ends with nothing left to do says how many quests it could not drive instead of announcing that everything is complete. What it reports comes from an outcome written per quest at the moment the run decides it, completed, blocked or failed, rather than read back afterwards from `completedAt`: a quest that Discord completed while the run happened to be alive, or one the user finished by hand after Orion had skipped it, would otherwise be counted as something Orion finished, and could land in two columns at once. Completed overwrites an earlier blocked or failed and nothing overwrites completed, so a run that farmed one quest and skipped another still reports the one it finished and plays the sound for it.
